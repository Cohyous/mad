...
def create_app():
    ...
    with app.app_context():
        db.create_all()
        seed_books()
        librarian = app.security.datastore.find_or_create_role(
            name='librarian', description='Administrator')
        user = app.security.datastore.find_or_create_role(
            name='user', description='Customer')
        if not app.security.datastore.find_user(email="admin@gmail.com"):
            app.security.datastore.create_user(
                email="admin@gmail.com", username='admin', password=hash_password("password"), roles=[librarian])
        db.session.commit()
...
api.add_resource(BookFeedbacks, '/book_feedbacks')
...
api.add_resource(LibrarianUserRequestsAPI,
                 '/librarian/user_requests/<int:user_id>')
api.add_resource(LibrarianManageRequestAPI,
                 '/librarian/manage_request/<int:request_id>/<string:action>')
api.add_resource(LibrarianEditDeleteBookAPI,
                 '/librarian/edit_delete_book/<int:book_id>')
api.add_resource(LibrarianEditSectionAPI,
                 '/librarian/edit_section/<int:section_id>')
